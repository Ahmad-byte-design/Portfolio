...
function Hero() {
  ...
  return (
    <motion.div initial={{opacity:0}} whileInView={{opacity:1}} className="w-full min-h-screen  flex  gap-2">
      <motion.div initial={{x:-100 , opacity:0 }} whileInView={{x:0 ,opacity:1}} className="flex flex-col items-start p-2 justify-center gap-5 w-1/2 ">
        ...
        <motion.div initial={{opacity:0,scale:0}} whileInView={{opacity:1,scale:1}} whileTap={{scale:1.05}} className="w-1/2 relative flex items-center justify-center">
          <Image src={project}/>
        </motion.div>
    </motion.div>
  );
